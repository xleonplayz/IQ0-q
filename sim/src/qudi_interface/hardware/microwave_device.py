...
This module implements the MicrowaveInterface for the NV center simulator,
enabling ODMR and other microwave-driven experiments.
...
class NVSimMicrowaveDevice(MicrowaveInterface):
    ...
    def __init__(self, config=None, **kwargs):
        """
        Initialize the microwave device adapter for the NV simulator.
    ...
    def on_activate(self):
        ...
        try:
            self.log.info('NV Simulator microwave device activated')
            ...
            if 'magnetic_field' in self._config:
                magnetic_field = self._config['magnetic_field']
                self._qudi_facade.apply_global_environment(magnetic_field=magnetic_field)
                self.log.info(f"Applied magnetic field: {magnetic_field} Gauss")
                
            if 'temperature' in self._config:
                temperature = self._config['temperature']
                self._qudi_facade.apply_global_environment(temperature=temperature)
                self.log.info(f"Applied temperature: {temperature} K")
            
            # Apply strain if specified
            if 'strain' in self._config:
                strain = self._config['strain']
                self._qudi_facade.apply_global_environment(strain=strain)
                self.log.info(f"Applied strain: {strain}")
    ...
    def on_deactivate(self):
        ...
        try:
            ...
            self._simulator.apply_microwave(self._cw_frequency, self._cw_power, on=False)
            self.log.info('NV Simulator microwave device deactivated')
        finally:
            self.module_state.unlock()
    ...
    def scan_next(self) -> None:
        """
        Step to the next frequency in a scan.
